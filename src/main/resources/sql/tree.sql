WITH ROOT (ID, PARENT_NODE_ID, CHILD_NODE_ID, START_DATE, END_DATE, HIERARCHY) AS (
	...
	UNION
	SELECT 'en' FROM dual
) SELECT PARENT_NODE_ID, CHILD_NODE_ID, CHILD.UNIQUE_ID, HIERARCHY, LANGUAGES.LANGUAGE, COALESCE(FULL_NAME."VALUE", CHILD.NAME) AS NODE_NAME FROM ROOT 
		CROSS JOIN LANGUAGES 
		JOIN NODE CHILD ON ROOT.CHILD_NODE_ID = CHILD.ID
		LEFT JOIN NODE_ATTR FULL_NAME ON CHILD.ID = FULL_NAME.NODE_ID 
			AND 'name_' || LANGUAGES.LANGUAGE = FULL_NAME."KEY" 
			AND (FULL_NAME.START_DATE IS NULL OR TRUNC(FULL_NAME.START_DATE) <= TRUNC(TO_DATE(:date, 'DD.MM.YYYY')))
			AND (FULL_NAME.END_DATE IS NULL OR TRUNC(FULL_NAME.END_DATE) >= TRUNC(TO_DATE(:date, 'DD.MM.YYYY')))
