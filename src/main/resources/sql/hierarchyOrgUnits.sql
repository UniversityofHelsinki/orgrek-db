WITH START_FROM_HY (ID, PARENT_NODE_ID, CHILD_NODE_ID, START_DATE, END_DATE, HIERARCHY) AS (
	SELECT 
		ID, 
		PARENT_NODE_ID,
		CHILD_NODE_ID,
	...
	WHERE PARENT_NODE_ID IS NULL 
		AND CHILD_NODE_ID = :start
		AND HIERARCHY IN :hierarchy
		UNION ALL 
	SELECT 
		EDGE.ID, 
		EDGE.PARENT_NODE_ID,
		...
		EDGE.END_DATE, 
		EDGE.HIERARCHY 
	FROM START_FROM_HY JOIN EDGE ON START_FROM_HY.CHILD_NODE_ID = EDGE.PARENT_NODE_ID 
		AND START_FROM_HY.HIERARCHY = EDGE.HIERARCHY 
	...
	FROM NODE 
	JOIN EDGE EDGE ON EDGE.HIERARCHY = :hierarchy AND (NODE.ID = EDGE.CHILD_NODE_ID)
	...
	WHERE (PARENT.ID IS NOT NULL OR CODE.VALUE = 'HY')
