...
FROM EDGE E
         JOIN NODE N ON
            E.PARENT_NODE_ID = N.ID AND E.CHILD_NODE_ID = :nodeId AND E.HIERARCHY != 'history' AND
            (E.START_DATE IS NULL OR E.START_DATE <= trunc(to_date(:date, 'DD.MM.YYYY')))
         ...
         LEFT JOIN FULL_NAME PAST ON FUTURE.NODE_ID IS NULL AND E.PARENT_NODE_ID = PAST.NODE_ID AND
         ...
         LEFT JOIN FULL_NAME CURR ON PAST.NODE_ID IS NULL AND FUTURE.NODE_ID IS NULL AND E.PARENT_NODE_ID = CURR.NODE_ID AND
