SELECT PN.ID, P.ID AS EDGE_ID, PN.UNIQUE_ID, PN.START_DATE, PN.END_DATE, P.START_DATE edge_start_date, P.END_DATE edge_end_date, FN.LANGUAGE, max(fn.name) keep (dense_rank first order by fn.start_date desc nulls last, fn.end_date desc nulls first) full_name FROM PREDECESSOR_RELATION P
...
where P.NODE_ID = :nodeId group by PN.ID, P.ID, PN.UNIQUE_ID, PN.START_DATE, PN.END_DATE, P.START_DATE, P.END_DATE, FN.LANGUAGE
