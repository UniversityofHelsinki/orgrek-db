...
FROM EDGE E
         JOIN NODE N ON
            E.PARENT_NODE_ID = :nodeId AND E.CHILD_NODE_ID = N.ID AND E.HIERARCHY != 'history' AND
            (E.END_DATE IS NULL OR NOT (E.END_DATE < trunc(to_date(:date, 'DD.MM.YYYY'))))
         CROSS JOIN LANGUAGES
         LEFT JOIN FULL_NAME FUTURE ON E.CHILD_NODE_ID = FUTURE.NODE_ID AND LANGUAGES.LANGUAGE = FUTURE.LANGUAGE AND
         ...
         LEFT JOIN FULL_NAME PAST ON FUTURE.NODE_ID IS NULL AND E.CHILD_NODE_ID = PAST.NODE_ID AND
                    LANGUAGES.LANGUAGE = PAST.LANGUAGE AND
         ...
         LEFT JOIN FULL_NAME CURR ON FUTURE.NODE_ID IS NULL AND PAST.NODE_ID IS NULL AND E.CHILD_NODE_ID = CURR.NODE_ID AND
                    LANGUAGES.LANGUAGE = CURR.LANGUAGE AND
