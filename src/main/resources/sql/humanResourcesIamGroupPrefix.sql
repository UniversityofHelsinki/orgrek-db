SELECT 
	...
	IAM_GROUP.VALUE AS IAM_GROUP_PREFIX,
	PARENT.UNIQUE_ID AS PARENT
FROM EDGE 
JOIN NODE ON 
	EDGE.CHILD_NODE_ID = NODE.ID AND
...
LEFT JOIN NODE_ATTR CODE ON NODE.ID = CODE.NODE_ID AND 
...
JOIN NODE_ATTR TYPE ON NODE.ID = TYPE.NODE_ID AND
	TYPE.KEY = 'type' AND
...
JOIN HIERARCHY_FILTER TYPE_FILTER ON 
	TYPE_FILTER.HIERARCHY = 'henkilosto_iam' AND
...
LEFT JOIN NODE_ATTR IAM_GROUP ON
	NODE.ID = IAM_GROUP.NODE_ID AND
	IAM_GROUP.KEY = 'iam_ryhma_alkuosa' AND
	(IAM_GROUP.START_DATE IS NULL OR IAM_GROUP.START_DATE <= TRUNC(CURRENT_DATE)) AND
	(IAM_GROUP.END_DATE IS NULL OR IAM_GROUP.END_DATE >= TRUNC(CURRENT_DATE))
LEFT JOIN NODE PARENT ON 
	EDGE.PARENT_NODE_ID = PARENT.ID AND
...
WHERE 
	EDGE.HIERARCHY = 'henkilosto_iam' AND
