...
WITH ROOT (NAME, PARENT_NODE_ID, CHILD_NODE_ID, LEVEL_ONE) AS (
  ...
  FROM NODE 
    ...
    LEFT JOIN NODE_ATTR TULOSYKSIKKO
      ON NODE.ID = TULOSYKSIKKO.NODE_ID
      AND TULOSYKSIKKO.KEY = 'type'
      AND TULOSYKSIKKO.VALUE = 'tulosyksikko'
      AND (TULOSYKSIKKO.START_DATE IS NULL OR TULOSYKSIKKO.START_DATE <= TRUNC(CURRENT_DATE))
      AND (TULOSYKSIKKO.END_DATE IS NULL OR TULOSYKSIKKO.END_DATE >= TRUNC(CURRENT_DATE))
